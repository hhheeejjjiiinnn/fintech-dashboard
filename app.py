...
with tab2:
    ...
    with c1:
        st.markdown("#### 채널별 CTR (%)  ")
        fig4 = px.bar(ch.sort_values("CTR(%)"), x="CTR(%)", y="channel", orientation="h",
    ...
    with c2:
        ...
        for _, row in ch.iterrows():
            fig5.add_trace(go.Bar(name=f"{row['channel']} 노출", x=[row["channel"]],
                y=[row["노출"]/1e6], marker_color=COLORS.get(row["channel"],"#888"), opacity=0.5))
            fig5.add_trace(go.Bar(name=f"{row['channel']} 클릭", x=[row["channel"]],
...
with tab5:
    ...
    insights = [
        ("1", "구글이 CPA 최저 — 예산 재배분 기회", "#4285f4",
         "구글 CPA **507원**은 네이버검색(1,849원) 대비 **3.6배 저렴**. 현재 구글 예산 비중은 25%에 불과하나 CPA 기준 최고 효율 채널. 구글 예산 확대 시 전환당 비용 약 **40% 절감** 가능."),
        ...
        ("5", "퍼널 최대 누수 — 클릭→설치(56%)가 핵심 게이트", "#f6e05e",
         "클릭 4,624만 중 설치 2,595만(56.1%)으로 절반만 설치. 설치 이후는 실행 90% → 가입 77% → 계좌 77%로 비교적 양호. **랜딩페이지 최적화**와 앱스토어 페이지 개선이 최우선 과제."),
        ...
        ("10", "회원가입·계좌개설 예산 균등 — 계좌개설 확대 여지", "#f687b3",
         "회원가입(133.6억) vs 계좌개설(134.9억)이 거의 **50:50** 배분. 계좌개설이 더 높은 LTV로 이어지므로 계좌개설 캠페인 비중을 **60:40**으로 확대 시 장기 수익성 향상 예상.")
    ]
